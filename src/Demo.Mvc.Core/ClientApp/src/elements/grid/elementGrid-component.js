      ...
      <div className="row mw-grid">
        {element.childs.map((childElement, $index) => (<>
            <GridElement element={childElement} />
              { ($index+1)%2 ===0 && (<div className="clearfix visible-sm-block"></div>)}
              { ($index+1)%3 ===0 && (<div className="clearfix visible-md-block"></div>)}
              { ($index+1)%4 ===0 && (<div className="clearfix visible-lg-block"></div>)}
        </>))}
      </div>
  );
};
