...
export const isDisplayMenu = search => {
...
const getUrlSite =(baseUrlSite, path) => concatUrl(baseUrlSite, path);

const initMenu = (menuItemsSource, baseUrlSite) => {
...
const init=(masterTemp, baseUrlSite) => {
  ...
  for (let i = 0; i < masterTemp.master.imageLogos.length; i++) {
...
const _initHomePageInfo= (masterTemp, menuKey) => {
  let moduleName;
  const homePageInfo = {};
