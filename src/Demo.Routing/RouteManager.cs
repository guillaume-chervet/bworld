...
namespace Demo.Routing
{
    ...
    public class RouteManager : IRouteManager
    {
        ...
        public RouteManager(IRouteProvider routeProvider, ICacheRepository cacheRepository, ILogger<RouteManager> logger)
        {
        ...
        private async Task<bool> IsHomePage(Domain domain, IDictionary<string, string> domainDatas,
            ...
            IDictionary<string, string> routeDatas)
        {
            ...
            {
                if (!routeDatas.ContainsKey(metadata.Key))
                {
                    return false;
                }
                if (routeDatas[metadata.Key] != metadata.Value)
                {
                    return false;
                }
            }
        ...
        /// <returns></returns>
        private static bool IsSameDomainType(IDictionary<string, string> domainDatas, Domain domain2)
        {
            ...
            foreach (var key in data2.Keys)
            {
                if (!domainDatas.ContainsKey(key))
                    return false;
                if(domain2.ExcludedDomainData != null && domain2.ExcludedDomainData.ContainsKey(key) && domain2.ExcludedDomainData[key] == domainDatas[key])
                {
                    return false;
                }
            }
        ...
        private static IDictionary<string, string> GetDomainDatas(Domain domain1)
        {
        ...
        /// <param name="domainDatas"> </param>
        /// <returns></returns>
        private static string GetRoutePath(string url, Domain domain, Dictionary<string, string> domainDatas)
        {
            var routePath = GetPath(url, domainDatas, null);
            var domainPath = string.Empty;

            if (!string.IsNullOrEmpty(domain.Path))
                domainPath = GetPath(domain.Path, domainDatas, null);
        ...
        private static async Task<Route> LoadRouteAsync(string url, IDictionary<string, string> data,
            IRouteProvider routeProvider, Dictionary<string, string> domainDatas, string siteId, ILogger<RouteManager> logger)
        {
            ...
            foreach (var route in routes)
            {
                ...
                if (CheckRoute(url, data, route, path))
                {
                    return route;
                }
            }
        ...
        private static Route RouteResult(IDictionary<string, string> data, IRouteProvider routeProvider, Dictionary<string, string> domainDatas,
