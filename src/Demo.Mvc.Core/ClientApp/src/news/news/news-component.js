          ...
          <div className="panel panel-default">
            <div className="panel-heading">
              <h3 className="panel-title">
                <a className="mw-title" href={item.data.viewUrl}>{item.data.title}</a>
                {vm.isDraft(item.data) ? (<span className="label label-default">Brouillon</span>) : null}
                {vm.isDeleted(item.data) ? (<span className="label label-danger">Supprimé</span>) : null}
                <Tags tags={item.data.tags} />
              </h3>
            </div>
            <div>
              <a href={item.data.viewUrl} title={item.data.title} className="hand"><img
                  src={vm.getFirstImage(item.element).thumbnailUrl}
                  className="center-block img-responsive img-thumbnail"
                  alt={vm.getFirstImage(item.element).name}/></a>
            </div>
          </div>
        </div>
        {(items.indexOf(item)+1)%2 ===0 ? (<div className="clearfix visible-sm-block"></div>) : null}
        {(items.indexOf(item)+1)%2 ===0 ? (<div className="clearfix visible-md-block"></div>) : null}
        {(items.indexOf(item)+1)%3 ===0 ? (<div className="clearfix visible-lg-block"></div>) : null}
      </div>
    )) }
    ...
    {items.map((item) => (
        <div className="panel panel-default" key={item.data.viewUrl}>
          ...
          <div className="panel-body">
            ...
            {item.data.hasNext ? (<a className="pull-right btn btn-default btn-lg" href={item.data.viewUrl}>Lire la suite</a>) : null }
          </div>
...
const News = ({vm, onTagChange}) => {
  const Content = vm.data.getDisplayMode() === "galerie" ? SwitchGallerie : SwitchDefault;
  return (<PageBreadcrumbWithState className="mw-news">
      ...
      <div className="row">
        <div className="col-sm-9">
          <Empty items={vm.items} text={'Aucun article.'} />
            ...
            <ul className="pager">
              {vm.data.hasPrevious() ? (<li className="previous"><a href={vm.data.urlPrevious}><span aria-hidden="true">&larr;</span> Précédent</a></li>) : null}
              {vm.data.hasNext() ? (<li className="next"><a href={vm.data.urlNext}>Suivant<span aria-hidden="true">&rarr;</span></a></li>) : null}
            </ul>
          </nav>): null}
        </div>
          ...
          <div className="panel panel-default">
            ...
            <div className="panel-body">
              <div className="form-group" nf-if="vm.tags.length>0">
                ...
                { vm.tags.map((tag) => (<div key={tag.id} className="checkbox" >
                  <label>
                    <input value={tag.ticked} type="checkbox" onChange={() => onTagChange(tag)} /> {tag.name}
                  </label>
                </div>))}
              </div>
              <button className="btn btn-lg btn-default pull-right" type="button" onClick={vm.filter}> Filtrer
              </button>
            </div>
          </div>
        </div>
